...
# Print summary of failed test cases with WPEFramework logs
if ReportGenerator.failed_tc_list:
    print("\n" + "="*100)
    print("FAILED TEST CASES SUMMARY WITH WPEFramework LOGS")
    print("="*100)
    for failed_tc in ReportGenerator.failed_tc_list:
        print(f"\nFailed Test Case: {failed_tc}")
        try:
            wpe_logs = Utils.get_wpeframework_error_logs(failed_tc, num_lines=30)
            print(wpe_logs)
        except Exception as e:
            print(f"Error retrieving logs for {failed_tc}: {str(e)}")
        print("-"*100)
    print("\n")
