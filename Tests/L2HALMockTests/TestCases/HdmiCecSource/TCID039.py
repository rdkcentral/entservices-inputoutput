...
# Testcase ID : TCID039
# Testcase Description : Set the vendor id with missing parameter and verify that
# error response is obtained

from Utilities import Utils, ReportGenerator
from HdmiCecSource import HdmiCecSourceApis

print("TC Description - Set the vendor id with missing parameter and verify that error response is obtained")
Utils.initiliaze_flask_for_HdmiCecSource()
print("---------------------------------------------------------------------------------------------------------------------------")

# send the curl command to set the vendor id with missing parameter
set_response = Utils.send_curl_command(HdmiCecSourceApis.set_vendor_id_invalid_2)
if set_response:
    Utils.info_log("curl command sent for setting the vendor id with missing parameter")
else:
    Utils.error_log("curl command failed for setting the vendor id with missing parameter")
print("")
...
tc_id = 'TCID039_setVendorId_MissingParameter'
