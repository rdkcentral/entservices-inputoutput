...
# Testcase ID : TCID040
# Testcase Description : Set the OSD name with missing parameter and verify that
# error response is obtained

from Utilities import Utils, ReportGenerator
from HdmiCecSource import HdmiCecSourceApis

print("TC Description - Set the OSD name with missing parameter and verify that error response is obtained")
Utils.initiliaze_flask_for_HdmiCecSource()
print("---------------------------------------------------------------------------------------------------------------------------")

# send the curl command to set the OSD name with missing parameter
set_response = Utils.send_curl_command(HdmiCecSourceApis.set_osd_name_invalid)
if set_response:
    Utils.info_log("curl command sent for setting the OSD name with missing parameter")
else:
    Utils.error_log("curl command failed for setting the OSD name with missing parameter")
print("")
...
tc_id = 'TCID040_setOSDName_MissingParameter'
