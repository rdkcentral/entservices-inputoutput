...
# Testcase ID : TCID037
# Testcase Description : Hit the curl command for performOTPAction and verify that
...
print("TC Description - Hit the curl command for performOTPAction and verify that output response is correct")
...
if str(curl_response) == str(expected_output_response):
    status = 'Pass'
    message = 'Output response is matching with expected one. The performOTPAction is successful'
else:
    status = 'Fail'
    message = 'Output response is different from expected one'

# generate logs in terminal
tc_id = 'TCID037_performOTPAction_Standalone'
