...
/**
 * @brief Test GetEnabled via COM-RPC
 * 
 * This test verifies that the GetEnabled method works correctly through COM-RPC interface.
 * It checks if the HdmiCecSource plugin is enabled or disabled and validates the response.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                TEST_LOG("Enabled value: %d", enabled);
...
/**
 * @brief Test GetEnabled via JSON-RPC
 * 
 * This test verifies that the getEnabled method works correctly through JSON-RPC interface.
 */
...
/**
 * @brief Test SetEnabled via COM-RPC
 * 
 * This test verifies that the SetEnabled method works correctly through COM-RPC interface.
 * It enables the HdmiCecSource plugin and validates the response.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                HdmiCecSourceSuccess successResult;
                
                // Call the API
                uint32_t result = m_cecSourcePlugin->SetEnabled(enableValue, successResult);
                ...
                TEST_LOG("SetEnabled success: %d", successResult.success);
...
/**
 * @brief Test SetEnabled via JSON-RPC
 * 
 * This test verifies that the setEnabled method works correctly through JSON-RPC interface.
 */
...
/**
 * @brief Test GetOSDName via COM-RPC
 * 
 * This test verifies that the GetOSDName method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Log and validate output
                TEST_LOG("OSD Name: %s", osdName.c_str());
...
/**
 * @brief Test GetOSDName via JSON-RPC
 * 
 * This test verifies that the getOSDName method works correctly through JSON-RPC interface.
 */
TEST_F(HdmiCecSource_L2Test, GetOSDName_JSONRPC)
{
    ...
    if (result.HasLabel("name")) {
        string name = result["name"].String();
        TEST_LOG("  name: %s", name.c_str());
        EXPECT_FALSE(name.empty());
    }
}

/**
 * @brief Test SetOSDName via COM-RPC
 * 
 * This test verifies that the SetOSDName method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Call the API
                uint32_t result = m_cecSourcePlugin->SetOSDName(osdName, successResult);
                ...
                EXPECT_TRUE(successResult.success);
                TEST_LOG("SetOSDName success: %d", successResult.success);
...
/**
 * @brief Test SetOSDName via JSON-RPC
 * 
 * This test verifies that the setOSDName method works correctly through JSON-RPC interface.
 */
...
/**
 * @brief Test GetVendorId via COM-RPC
 * 
 * This test verifies that the GetVendorId method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Log and validate output
                TEST_LOG("Vendor ID: %s", vendorId.c_str());
...
/**
 * @brief Test GetVendorId via JSON-RPC
 * 
 * This test verifies that the getVendorId method works correctly through JSON-RPC interface.
 */
...
/**
 * @brief Test SetVendorId via COM-RPC
 * 
 * This test verifies that the SetVendorId method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Set vendor ID
                string vendorId = "0019FB";
                HdmiCecSourceSuccess successResult;
                
                // Call the API
                uint32_t result = m_cecSourcePlugin->SetVendorId(vendorId, successResult);
                ...
                EXPECT_TRUE(successResult.success);
                TEST_LOG("SetVendorId success: %d", successResult.success);
...
/**
 * @brief Test SetVendorId via JSON-RPC
 * 
 * This test verifies that the setVendorId method works correctly through JSON-RPC interface.
 */
...
/**
 * @brief Test GetOTPEnabled via COM-RPC
 * 
 * This test verifies that the GetOTPEnabled method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Log and validate output
                TEST_LOG("OTP Enabled: %d", enabled);
...
/**
 * @brief Test GetOTPEnabled via JSON-RPC
 * 
 * This test verifies that the getOTPEnabled method works correctly through JSON-RPC interface.
 */
...
/**
 * @brief Test SetOTPEnabled via COM-RPC
 * 
 * This test verifies that the SetOTPEnabled method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Call the API
                uint32_t result = m_cecSourcePlugin->SetOTPEnabled(enableValue, successResult);
                ...
                EXPECT_TRUE(successResult.success);
                TEST_LOG("SetOTPEnabled success: %d", successResult.success);
...
/**
 * @brief Test SetOTPEnabled via JSON-RPC
 * 
 * This test verifies that the setOTPEnabled method works correctly through JSON-RPC interface.
 */
...
/**
 * @brief Test GetActiveSourceStatus via COM-RPC
 * 
 * This test verifies that the GetActiveSourceStatus method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Call the API
                uint32_t result = m_cecSourcePlugin->GetActiveSourceStatus(status, success);
                ...
                // Log and validate output
                TEST_LOG("Active Source Status: %d", status);
...
/**
 * @brief Test GetActiveSourceStatus via JSON-RPC
 * 
 * This test verifies that the getActiveSourceStatus method works correctly through JSON-RPC interface.
 */
TEST_F(HdmiCecSource_L2Test, GetActiveSourceStatus_JSONRPC)
{
    ...
    if (result.HasLabel("status")) {
        bool activeStatus = result["status"].Boolean();
        TEST_LOG("  status: %d", activeStatus);
    }
}

/**
 * @brief Test PerformOTPAction via COM-RPC
 * 
 * This test verifies that the PerformOTPAction method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Call the API
                uint32_t result = m_cecSourcePlugin->PerformOTPAction(successResult);
                ...
                EXPECT_TRUE(successResult.success);
                TEST_LOG("PerformOTPAction success: %d", successResult.success);
...
/**
 * @brief Test PerformOTPAction via JSON-RPC
 * 
 * This test verifies that the performOTPAction method works correctly through JSON-RPC interface.
 */
...
/**
 * @brief Test SendStandbyMessage via COM-RPC
 * 
 * This test verifies that the SendStandbyMessage method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Call the API
                uint32_t result = m_cecSourcePlugin->SendStandbyMessage(successResult);
                ...
                EXPECT_TRUE(successResult.success);
                TEST_LOG("SendStandbyMessage success: %d", successResult.success);
...
/**
 * @brief Test SendStandbyMessage via JSON-RPC
 * 
 * This test verifies that the sendStandbyMessage method works correctly through JSON-RPC interface.
 */
...
/**
 * @brief Test SendKeyPressEvent via COM-RPC
 * 
 * This test verifies that the SendKeyPressEvent method works correctly through COM-RPC interface.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                uint32_t keyCode = 0x44;      // Volume Up key code
                HdmiCecSourceSuccess successResult;
                
                // Call the API
                uint32_t result = m_cecSourcePlugin->SendKeyPressEvent(logicalAddress, keyCode, successResult);
                ...
                EXPECT_TRUE(successResult.success);
                TEST_LOG("SendKeyPressEvent success: %d", successResult.success);
...
/**
 * @brief Test SendKeyPressEvent via JSON-RPC
 * 
 * This test verifies that the sendKeyPressEvent method works correctly through JSON-RPC interface.
 */
TEST_F(HdmiCecSource_L2Test, SendKeyPressEvent_JSONRPC)
{
    ...
    params["keyCode"] = 0x44;         // Volume Up key code
    JsonObject result;
...
/**
 * @brief Test GetDeviceList via COM-RPC
 * 
 * This test verifies that the GetDeviceList method works correctly through COM-RPC interface.
 * It retrieves the list of CEC devices connected to the network.
 */
    ...
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        ...
        if (m_controller_cecSource) {
            ...
            if (m_cecSourcePlugin) {
                ...
                // Log and validate output
                TEST_LOG("Number of devices: %u", numberOfDevices);
                
                if (deviceList != nullptr) {
                    HdmiCecSourceDevice device;
                    uint32_t deviceIndex = 0;
                    
                    while (deviceList->Next(device) == true) {
                        TEST_LOG("Device %u:", deviceIndex++);
                        TEST_LOG("  Logical Address: %u", device.logicalAddress);
                        TEST_LOG("  Vendor ID: %s", device.vendorID.c_str());
                        TEST_LOG("  OSD Name: %s", device.osdName.c_str());
                        
                        // Validate device data
                        EXPECT_GE(device.logicalAddress, 0);
                        EXPECT_LE(device.logicalAddress, 15);
                    }
                    
                    deviceList->Release();
                } else {
                    TEST_LOG("Device list is NULL");
                }
...
/**
 * @brief Test GetDeviceList via JSON-RPC
 * 
 * This test verifies that the getDeviceList method works correctly through JSON-RPC interface.
 */
TEST_F(HdmiCecSource_L2Test, GetDeviceList_JSONRPC)
{
    ...
    if (result.HasLabel("numberofdevices")) {
        ...
        TEST_LOG("  numberOfDevices: %u", numberOfDevices);
    }
    
    // Validate deviceList field
    EXPECT_TRUE(result.HasLabel("deviceList"));
    if (result.HasLabel("deviceList")) {
        JsonArray deviceList = result["deviceList"].Array();
        TEST_LOG("  deviceList array size: %zu", deviceList.Length());
        ...
        for (uint32_t i = 0; i < deviceList.Length(); i++) {
            ...
            if (device.HasLabel("logicalAddress")) {
                uint8_t logicalAddress = device["logicalAddress"].Number();
                TEST_LOG("    logicalAddress: %u", logicalAddress);
                EXPECT_GE(logicalAddress, 0);
                EXPECT_LE(logicalAddress, 15);
            }
...
/**
 * @brief Test SetEnabled with false value via COM-RPC
 * 
 * This test verifies that the SetEnabled method can disable the HdmiCecSource plugin.
 */
TEST_F(HdmiCecSource_L2Test, SetEnabled_False_COMRPC)
{
    if (CreateHdmiCecSourceInterfaceObject() != Core::ERROR_NONE) {
        TEST_LOG("Invalid HdmiCecSource_Client");
    } else {
        ...
        if (m_controller_cecSource) {
            EXPECT_TRUE(m_cecSourcePlugin != nullptr);
            if (m_cecSourcePlugin) {
                TEST_LOG("Testing SetEnabled(false) via COM-RPC");
                ...
                EXPECT_TRUE(successResult.success);
                TEST_LOG("SetEnabled(false) success: %d", successResult.success);
...
/**
 * @brief Test SetEnabled with false value via JSON-RPC
 * 
 * This test verifies that the setEnabled method can disable the HdmiCecSource plugin through JSON-RPC.
 */
TEST_F(HdmiCecSource_L2Test, SetEnabled_False_JSONRPC)
{
    TEST_LOG("Testing setEnabled(false) via JSON-RPC");
